...
function findRepo() {
  ...
  while (true) {
    if (existsSync(join(dir, 'content')) && existsSync(join(dir, 'schema'))) return dir;
    const up = dirname(dir);
    if (up === dir) throw new Error('style-repo root not found from ' + process.cwd());
    dir = up;
  }
}
const REPO = findRepo();
...
export function loadAll() {
  ...
  const terms = [];
  for (const file of readdirSync(CONTENT).filter((f) => f.endsWith('.yaml')).sort()) {
  ...
  const push = (from, relation, to, inferred) => {
    if (byId.has(from) && byId.has(to)) relations.push({ from, relation, to, inferred });
  };
...
export function thumb(term) {
  for (const img of term.images ?? []) {
    if (typeof img?.file === 'string' && img.file) return { src: '/' + img.file, img };
    if (typeof img?.remote_image === 'string') return { src: img.remote_image, img };
  }
  return null;
}
...
export function lifecycleRows() {
  ...
  return terms
    ...
    .map((t) => {
      ...
      const flag = !lc.review_due ? 'no date' : lc.review_due <= today ? 'overdue' : 'ok';
      return { term: t, state: lc.state, review_due: lc.review_due ?? null, flag };
    });
}

export function trendQueue() {
  const path = join(REPO, 'trends/queue.yaml');
  if (!existsSync(path)) return { pending: [], counts: { pending: 0, dismissed: 0, accepted: 0 } };
  const q = parse(readFileSync(path, 'utf8')) ?? {};
  const all = q.candidates ?? [];
  const counts = {
    pending: all.filter((c) => c.status === 'pending').length,
    dismissed: all.filter((c) => c.status === 'dismissed').length,
    accepted: all.filter((c) => String(c.status).startsWith('accepted:')).length,
  };
...
export function facetDimensions() {
  ...
  const dims = [
    ...
    ['context', 'Used for', (t) => t.facets?.commercial_context?.contexts ?? []],
