...
let relCount = 0, imgCount = 0;
for (const { doc } of entries) {
  const f = doc.facets ?? {};
  const lc = f.lifecycle ?? {};
  insTerm.run(
    doc.id, doc.term_type, doc.status, doc.names?.primary ?? doc.id,
    doc.summary ?? null, f.ideological_stance ?? null,
    lc.state ?? null, lc.first_observed ?? null, lc.review_due ?? null,
    f.revival?.status ?? null, JSON.stringify(doc),
  );
  insFts.run(doc.id, doc.names?.primary ?? doc.id,
    (doc.names?.variants ?? []).join(' '), doc.summary ?? '');

  insFacet.run(doc.id, 'term_type', doc.term_type);
  for (const t of doc.secondary_types ?? []) insFacet.run(doc.id, 'term_type', t);
  const formal = f.formal_properties ?? f.formal_consequences ?? {};
  for (const k of ['grid_discipline', 'contrast', 'texture', 'hierarchy', 'ornament_level']) {
  ...
  if (f.ideological_stance) insFacet.run(doc.id, 'stance', f.ideological_stance);
  for (const c of f.commercial_context?.contexts ?? []) insFacet.run(doc.id, 'commercial_context', c);
  if (lc.state) insFacet.run(doc.id, 'lifecycle', lc.state);

  for (const [rel, targets] of Object.entries(f.lineage ?? {})) {
    for (const to of targets ?? []) {
      insRel.run(doc.id, rel, to, 0); relCount++;
      if (INVERSE[rel]) { insRel.run(to, INVERSE[rel], doc.id, 1); relCount++; }
    }
  }
  for (const to of f.production?.apparatus ?? []) {
    insRel.run(doc.id, 'uses_apparatus', to, 0);
    insRel.run(to, 'apparatus_used_by', doc.id, 1); relCount += 2;
  }
  for (const to of f.production?.techniques ?? []) {
    insRel.run(doc.id, 'uses_technique', to, 0);
    insRel.run(to, 'technique_used_by', doc.id, 1); relCount += 2;
  }

  for (const p of f.era?.periods ?? []) {
    insPeriod.run(doc.id, p.label, p.role, p.start_year ?? null, p.end_year ?? null);
  }
  for (const img of doc.images ?? []) {
    insImage.run(doc.id, img.file, img.source_url, img.license,
      img.attribution, img.rights_status, img.caption ?? null);
    imgCount++;
  }
}

db.close();
console.log(`build-index: OK — ${entries.length} terms, ${relCount} relations, ${imgCount} images → ${dbPath}`);
